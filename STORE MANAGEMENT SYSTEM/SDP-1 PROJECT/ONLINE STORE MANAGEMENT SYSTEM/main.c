...
//meed correction for login and registration
/*
void login()
{
...
void login()
{
	...
	if(opt == 1){
		...
		fwrite(&user,sizeof(user1),1,fp1);
		if(fwrite != 0){
		...
		if(fp == NULL) printf("Account number not registered");
			...
			fread(&user,sizeof(struct user1),1,fp1);
			fclose(fp1);
