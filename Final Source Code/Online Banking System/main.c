...
int main(){
	...
	if(opt == 1){
        ...
        {
            printf("Your password must be 5 digits\nTry again...\n");
        }
		}
		else
        {
            printf("Your Phone number must be 11 digits\nTry again...\n");
        }
	}
	else if(opt == 2){
		system("cls");
		...
		if(fp == NULL) printf("Account number not registered");
			...
			if(!strcmp(password,user.password)){
				while(cont == 'y'){
				system("cls");
				printf("\n\tWelcome %s %s",user.name,user.name1);
				...
				switch(choice){
					...
					case 3:
						...
						scanf("%d",&amount);
						if(amount<0) printf("\nSorry amount should be Greater than 0");
					...
					case 5:
						...
						break;
                    			case 7:
						printf("Your name is: %s %s",user.name,user.name1);
